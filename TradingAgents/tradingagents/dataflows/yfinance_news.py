...
def _fetch_vietnam_news(ticker: str, limit: int = 5) -> str:
    """Fetch live Vietnamese financial news using DDGS."""
    try:
        from ddgs import DDGS
        clean = ticker.replace(".VN", "").replace("^", "").strip()
        query = f"cổ phiếu {clean}"
        ...
        for r in results:
            title = r.get("title", "")
            source = r.get("source", "Báo chí VN")
            date = r.get("date", "")
            body = r.get("body", "")
            url = r.get("url", "")
            snippet = f"### {title} (Nguồn: {source} | {date})\n{body}\nLink: {url}"
            items.append(snippet)
        return "\n\n".join(items)
    except Exception as exc:
        logger.warning("Failed to fetch Vietnam news via DDGS: %s", exc)
        return ""
...
def get_news_yfinance(
    ...
    try:
        ...
        if filtered_count == 0 or canonical.endswith(".VN"):
            vn_news = _fetch_vietnam_news(ticker, limit=article_limit)
            if vn_news:
                return f"## Tin tức thị trường Việt Nam cho mã {ticker}{resolved} (cập nhật mới nhất):\n\n{vn_news}"
